...
var tests = [
  ...
  { id: 'lucky-color', name: '幸运色测试', emoji: '🌈', color: '#EC4899', desc: '什么颜色最旺你？',
    ...
    calcResult: function(answers) {
      ...
      var _c = colors[total % 4]; return { value: _c.value, desc: _c.desc, emoji: '🎨' };
    }
  },
  { id: 'superpower', name: '超能力测试', emoji: '⚡', color: '#F59E0B', desc: '你最适合什么超能力？',
    ...
    calcResult: function(answers) {
      ...
      var total = answers.reduce(function(s, a) { return s + a; }, 0)
      var _p = powers[total % 4]; return { value: _p.value, desc: _p.desc, emoji: _p.emoji };
    }
  },
  { id: 'animal', name: '灵魂动物测试', emoji: '🦊', color: '#059669', desc: '你的灵魂动物是什么？',
    ...
    calcResult: function(answers) {
      ...
      var total = answers.reduce(function(s, a) { return s + a; }, 0)
      var _a = animals[total % 4]; return { value: _a.value, desc: _a.desc, emoji: _a.emoji };
    }
  }
];
...
Page({
  onLoad: function() {
    ...
    analytics.startStay('fun');
    analytics.trackFunnelStart('fun');
    analytics.trackToolUse('fun');
  },
  data: {
    mode: 'list',
    tests: tests.map(function(t) { return { id: t.id, name: t.name, emoji: t.emoji, color: t.color, desc: t.desc }; }),
    currentTest: null,
    currentQuestion: 0,
    answers: [],
    showResult: false,
  ...
  handleAnswer: function(e) {
    ...
    var currentTest = this.data.currentTest; var currentQuestion = this.data.currentQuestion; var answers = this.data.answers;
    ...
    if (currentQuestion < total - 1) {
      ...
      var result = currentTest.calcResult(newAnswers);
      this.setData({ answers: newAnswers, showResult: true,
      _funnelDone: true, progress: 100, result });
    analytics.trackFunnelComplete('fun');
    analytics.trackResultView('fun');
    }
  },
  ...
  onHide: function() { analytics.endStay('fun'); },


  onUnload: function() { analytics.endStay('fun'); },
